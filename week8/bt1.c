...
int DAG(Graph graph)
{
   ...
   {
     memset(visited, 0, sizeof(visited)); //cap nhat tat ca visit =0

       start = jval_i(vertex->key);              
       stack = new_dllist();
       dll_append(stack, new_jval_i(start)); //cho start vo stack
    
       while ( !dll_empty(stack) )
       {
	 node = dll_last(stack);  //lay node ra khoi stack
          u = jval_i(node->val);
          dll_delete_node(node);
          if (!visited[u])
          {
              visited[u] = 1;
              n = outdegree(graph, u, output);   //lay va xet cac mien lan can cua u
              for (i=0; i<n; i++)
              {
                  v = output[i];
