...
int da_co_trong_open(state x) {
	int result = 0;
	int i;
	for (i = front_o2;i < rear_o2;i++ )
		if (strcmp(open2[i].board,x.board) == 0) return 1;
	return result;
}
	...
	while ((rear_o - front_o) != 0) {
		...
		if (strcmp(goal.board,cur_s.board) == 0) {
			...
			if 	(i_da_co_open == 0 && i_da_co_closed == 0) { // Neu chua co thi them vao danh sach open
				open[rear_o++] = next_s; //
				open2[rear_o2++]= next_s;
				//	print_state(open2[rear_o2-1]);
				path[node_count++] = cha; // Ghi lai vi tri state cha cua state hien tai
			}
